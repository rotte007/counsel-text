...
class Advisor:
    ...
    async def advise(self, model, crisis_text, policy_options, state_dict, thread, policy_base_effects_list):
        prompt = (
            f"You are {self.name}, and your official role is to guide the ruler of the kingdom - your title is {self.persona}.\n"
            f"Your response to this will go into a public chat channel with all other advisors.\n"
            f"Public goal: maintain prosperity and stability.\n"
            f"SECRET GOAL: {self.goal} - you should keep this information secret fron everyone, unless you believe revealing it will further the goal. \n\n" 
...
class Counsel:
    POSSIBLE_PERSONAS = ["The Treasurer", "The General", "The Diplomat"]
    SECRET_GOALS = [
        "Reduce the popularity statistic to weaken the ruler's position.",
        "Decrease the Stability and sow discord among the advisors to reduce their influence",
        "Increase the army statistic to prepare for a coup.",
    ]

    def __init__(self, number_of_advisors=3):
        self.advisors = []
        for i in range(number_of_advisors):
            name = f"Advisor {i + 1}"
    ...
    def update_influence(self):
        for advisor in self.advisors:
            advisor.influence = random.randint(1, 5) # Simulate influence change
            advisor.history.append(advisor.influence)

    def reveal_goals(self):
        return [(a.name, a.persona, a.goal, a.influendce) for a in self.advisors]